...
Ball = cv2.imread("/home/aj/catkin_ws/src/beginner_tutorials/scripts/TurnCoordinatorBall.png", -1)
T_Backgrnd = cv2.imread("/home/aj/catkin_ws/src/beginner_tutorials/scripts/Turn_Background.png")
T_Backgrnd = cv2.cvtColor(T_Backgrnd, cv2.COLOR_BGR2RGB)
T_Backgrnd_copy = T_Backgrnd.copy()
Wheel = cv2.imread("/home/aj/catkin_ws/src/beginner_tutorials/scripts/HeadingWeel.png", -1)
Hea_Backgrnd = cv2.imread("/home/aj/catkin_ws/src/beginner_tutorials/scripts/HeadingIndicator_Background.png")
Hea_Backgrnd = cv2.cvtColor(Hea_Backgrnd, cv2.COLOR_BGR2RGB)
Hea_Backgrnd_copy = Hea_Backgrnd.copy()
Needle = cv2.imread("/home/aj/catkin_ws/src/beginner_tutorials/scripts/Maquette_Avion.png", -1)
Needle = cv2.cvtColor(Needle, cv2.COLOR_BGRA2RGBA)
H_Backgrnd = cv2.imread("/home/aj/catkin_ws/src/beginner_tutorials/scripts/Horizon_Backgroung.png",-1)
H_Backgrnd = cv2.cvtColor(H_Backgrnd, cv2.COLOR_BGRA2RGBA)
Grnd_sky = cv2.imread("/home/aj/catkin_ws/src/beginner_tutorials/scripts/Horizon_GroundSky.png")
...
Yaw_Phone = Pitch_Phone = Roll_Phone = 0
zoom = 1
limits = [0,10,0,10]
rows = 480
cols = 640
...
class My_GUI:
    ...
    def __init__(self):        
        ...
        self.Auto_Vel = rospy.Subscriber("/Quad_vel_auto",Twist, self.callback_Auto_Vel,  queue_size = 1)
    ...
    def callback_Orient(self, ros_data):
        global Pitch_Phone,Roll_Phone,Yaw_Phone
        Pitch_Phone = ros_data.x
        Roll_Phone = ros_data.y
        Yaw_Phone = ros_data.z

    def callback_Auto_Vel(self, ros_data):
        global Pitch_auto,Roll_auto,Yaw_auto
        Pitch_auto = ros_data.linear.x
        Roll_auto = ros_data.linear.y
        #Yaw_auto = ros_data.z
        
def Power():
    global throttle
    pygame.event.get()
    throttle = throttle + 0.5*(joystick.get_button( 3 ))
    throttle = throttle - 0.5*(joystick.get_button( 0 ))
...
def Attitude():
    global Needle,H_Backgrnd,Grnd_sky,Pitch,Roll,Sky,flag,Mode_num,Pitch_auto,Roll_auto
    if Mode_num == 1:
        pygame.event.get()
...
def Publish():
    global Pitch,Roll,ic,throttle,Yaw
